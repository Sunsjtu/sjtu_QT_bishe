...
void MainWindow::on_comboBox_AxSelect_currentIndexChanged(int index)
{
    iComboXYZ = index;
}
...
void MainWindow::on_pushButton_AxInit_clicked()
{
    ...
    u32Return = Acm_AxSetSvOn(pAxisHandle[2], 1);
    //将轴0，1，2添加到群组中
...
void MainWindow::on_pushButton_AxStop_clicked()
{
    ...
    for (uint u32AxisNum=0; u32AxisNum<u32AxisCount; u32AxisNum++)
    {
        u32Return = Acm_AxSetSvOn(pAxisHandle[u32AxisNum], 0);
...
void MainWindow::on_holelocation_clicked()
{
    ...
    cv::circle(mask,center[0],530,cv::Scalar(255),-1);
    ...
    for(int j=0; j<maskimg.rows; j++)
    {
        ...
        for(int i=0; i<maskimg.cols; i++)
        {
            ...
            else
            {
                ...
                PPU_hole[num-1][0] = PPU_now[1] + (pix_x[num-1]-1226.6)*49.087 - 127570;
                PPU_hole[num-1][1] = PPU_now[2] - (pix_y[num-1]-1030)*49.087 + 530;
            }
        }
    }
    hole_n = num;//孔数设置
    ...
    for(int i=0; i<num; i++)
    {
        ...
        str.append("["+QString::number(PPU_hole[i][0])+","+QString::number(PPU_hole[i][1])+"]  ");
    }
...
//    indiedai.open("diedai.txt", ios::trunc);
    for (int i = 0; i < NcMax; i++)
    {
        ...
        int localTour[N][2];
        //局部最优长度
        long double localBestLength = 0.0;
        //当前路径长度
        long double tourLength;
        for (int j = 0; j < M; j++)
        {
...
//    inroute.open("route.txt", ios::trunc);
    ...
    //inroute.close();
    cout << endl;
    double path[N][2];//最终坐标点顺序
    int nPath;
    for (int m = 0; m < N; m++)
    {
        nPath = globalTour[m][0];
        path[m][0] = PPU_hole[nPath][0];
        path[m][1] = PPU_hole[nPath][1];
    }
    time(&timerl);
    int t = timerl - timer;

    double m_End[2] = { -8000, 28000 };
    U32 ArrayElementCnt = 2;
        //设置群组参数
    u32Return = Acm_SetF64Property(pGroupHandle, PAR_GpVelLow, 5000);
    u32Return = Acm_SetF64Property(pGroupHandle, PAR_GpVelHigh, 10000);
    u32Return = Acm_SetF64Property(pGroupHandle, PAR_GpAcc, 20000);
    u32Return = Acm_SetF64Property(pGroupHandle, PAR_GpDec, 20000);
    u32Return = Acm_SetF64Property(pGroupHandle, PAR_GpJerk, 0);
    double m_Start[2];
    m_Start[0] = path[0][0];
    m_Start[1] = path[0][1];
    U16 GpState;
    u32Return = Acm_GpGetState(pGroupHandle, &GpState);
    cout << GpState << endl;
    u32Return = Acm_GpMoveLinearAbs(pGroupHandle, m_Start, &ArrayElementCnt);
    cout << u32Return << endl;
    //判断轴状态，不为ready时延时
    do
    {
    ...
    //当前轴坐标
    double zAxis2, zAxis1;
    u32Return = Acm_AxGetActualPosition(pAxisHandle[1], &zAxis1);
    u32Return = Acm_AxGetActualPosition(pAxisHandle[2], &zAxis2);
    cout << zAxis1 << " " << zAxis2 << endl;


    //执行直线插补
    //ArrayElementCnt = m_ulAxisCount;
    for (int m = 1; m < N; m++)
    {
        m_End[0] = path[m][0];
        m_End[1] = path[m][1];
        u32Return = Acm_GpMoveLinearAbs(pGroupHandle, m_End, &ArrayElementCnt);
        cout << hex << u32Return << " ";
        do
        {
            u32Return = Acm_GpGetState(pGroupHandle, &GpState);
