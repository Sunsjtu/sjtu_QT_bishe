...
MVGIGE_API MVSTATUS_CODES __stdcall MVImageBayerToBGREx( HANDLE hCam,MV_IMAGE_INFO *pInfo,MVImage *pImage,double fGamma,bool bColorCorrect,int nContrast );
	...
	MVGIGE_API MVSTATUS_CODES __stdcall MVSetDebugParam(HANDLE hCam, int i, unsigned int v);
	MVGIGE_API MVSTATUS_CODES __stdcall MVGetDebugParam(HANDLE hCam, int i, unsigned int* pV);
	MVGIGE_API MVSTATUS_CODES __stdcall MVWriteMemory( HANDLE hCam, unsigned long aAddr, unsigned int aSize, void * aIn );
	MVGIGE_API MVSTATUS_CODES __stdcall MVReadMemory( HANDLE hCam, unsigned long aAddr, unsigned int aSize, void * aOut );
	MVGIGE_API MVSTATUS_CODES __stdcall MVUpgradeElf(HANDLE hCam,char *fname);
	MVGIGE_API MVSTATUS_CODES __stdcall MVUpgradeFir(HANDLE hCam,char *fname);
	MVGIGE_API int __stdcall MVGetCustomError(HANDLE hCam);
	 ...
	 *  \brief 设置传输模式，
	 *  \param [in]	hCam	相机句柄
	 *  \param [in]	mode	TransferControlMode_Basic:采集后立即传输，
							TransferControlMode_UserControlled:采集后暂存，用户发出MVTransferStart指令后才传输
	 *  \retval  	
	 */
	...
	MVGIGE_API MVSTATUS_CODES __stdcall MVTransferStart(HANDLE hCam);

	MVGIGE_API MVSTATUS_CODES __stdcall MVTransferStop(HANDLE hCam);
